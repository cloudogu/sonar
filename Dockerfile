...
RUN ls -lha sonarqube-8.9.8.54436
RUN curl --fail --location "https://github.com/cloudogu/sonar-cas-plugin/releases/download/v${CAS_PLUGIN_VERSION}/sonar-cas-plugin-${CAS_PLUGIN_VERSION}.jar" --output "./sonarqube-${SONAR_VERSION}/sonar-cas-plugin-${CAS_PLUGIN_VERSION}.jar"
RUN echo "${CAS_PLUGIN_JAR_SHA256} *${SONARQUBE_HOME}/sonar-cas-plugin-${CAS_PLUGIN_VERSION}.jar" | sha256sum -c -
...
COPY --from=builder --chown=1000:1000 /builder/sonarqube-${SONAR_VERSION} ${SONARQUBE_HOME}
